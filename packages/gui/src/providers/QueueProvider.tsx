...
export const QueueProvider: Component<{ children: JSX.Element }> = (props) => {
  ...
  const reorderItem = (itemId: string, toIndex: number, stopMiningCallback?: () => void) => {
    ...
    flushQueue(); // Phase 7: Persist immediately to avoid stale queue on refresh
  };
  ...
  const updateItemMiningState = (itemId: string, miningState: any) => {
    ...
    setQueueState((prev) => {
      const nextItems = prev.items.map((item) => {
        ...
        // Detect transition: default/empty nonces -> real nonces
        const prevLen = Array.isArray(item.miningState?.workerNonces) ? item.miningState.workerNonces.length : 0;
        const nextLen = Array.isArray(miningState?.workerNonces) ? miningState.workerNonces.length : 0;
        if (!shouldFlush && prevLen === 0 && nextLen > 0) {
          shouldFlush = true;
          debug('[Queue] First real nonces detected for', itemId, '— flushing queue state');
        }
        return { ...item, miningState };
      });
