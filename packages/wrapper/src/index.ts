...
export class Notemine {
  ...
  async resume(workerNonces?: string[]): Promise<void> {
    ...
    // If workerNonces provided, use them; otherwise use tracked nonces or derive from per-worker bests
    if (workerNonces && workerNonces.length > 0) {
      ...
      // Phase 9: Derive resume nonces from per-worker bestPow map if available
      const workersPow = this.workersPow$.getValue();
      const ids = workersPow ? Object.keys(workersPow) : [];
      if (ids.length > 0) {
        const derived: string[] = [];
        for (let i = 0; i < this.numberOfWorkers; i++) {
          const pow = (workersPow as any)[i] as BestPowData | undefined;
          derived[i] = pow?.nonce ?? i.toString();
        }
        this._resumeNonces = derived;
      }
    }
  ...
  restoreState(state: MiningState): void {
    ...
    // Restore nonces (treat empty array as no resume info)
    this._resumeNonces = (Array.isArray(state.workerNonces) && state.workerNonces.length > 0)
      ? state.workerNonces
      : undefined;
    ...
    if (state.workersPow) {
      ...
      if (workerPowValues.length > 0) {
        ...
        if (this._debug) {
          console.log('[Notemine] Phase 9: Restored per-worker POW map (' + workerPowValues.length + ' workers), highest:', maxPow.bestPow);
        }
      }
    } else if (state.bestPow) {
      // Phase 9: Legacy state compatibility
      // Do NOT set highestPow$ from legacy bestPow - let GUI handle it
      // The GUI will derive Highest Diff from current session workers only
      if (this._debug) {
        console.log('[Notemine] Phase 9: Legacy state detected (bestPow=' + state.bestPow.bestPow + ' but no workersPow map). Not setting highestPow$ - GUI will derive from current session.');
      }
    }
  ...
  private handleWorkerMessage(e: MessageEvent): void {
    ...
    if (type === 'initialized') {
      ...
      if (data?.bestPowData) {
        ...
        const workersPow = { ...this.workersPow$.getValue() } as Record<number, BestPowData>;
        const existing = workersPow[workerId];
        // Phase 9: Do not downgrade a worker's best across resume sessions
        if (!existing || bestPowData.bestPow >= existing.bestPow) {
